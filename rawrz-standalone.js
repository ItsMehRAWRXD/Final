...
class RawrZStandalone {
    ...
    async saveEncryptedFile(result, algorithm, extension = '.enc') {
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        const filename = `encrypted_${algorithm}_${timestamp}${extension}`;
        const filePath = path.join(this.uploadDir, filename);
        ...
        switch (command) {
            case 'encrypt':
                if (commandArgs.length < 2) {
                    ...
                    console.log('[INFO] Algorithms: aes256, aes128, blowfish, rsa2048, rsa4096');
            ...
            case 'keygen':
                if (commandArgs.length < 1) {
                    ...
                    console.log('[INFO] Algorithms: aes256, rsa2048, rsa4096');
                    console.log('[INFO] Save: true/false (default: false)');
